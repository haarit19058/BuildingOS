...
// Simple cprintf_console with %d, %x, %p, %s
pub unsafe fn cprintf_console(fmt: *const u8, args: *const usize, _nargs: usize) {
    let mut argp = args;
    let mut i = 0;
...
pub unsafe fn consoleread( mut dst: *mut u8, mut n: usize) -> isize {
    let target = n;
    let mut c: i32;
...
pub unsafe fn consolewrite( buf: *const u8, n: usize) -> isize {
    // iunlock(ip);
