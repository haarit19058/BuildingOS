...
// use crate::types_h::*; // for u32 aliases if you use them
use crate::arm_h;  // assume arm_h::trapframe exists

// Register offsets (word offsets; we index as VIC_BASE.add(offset))
const VIC_IRQSTATUS: u32 = 0;
const VIC_FIQSTATUS: u32 = 1;
const VIC_RAWINTR: u32 = 2;
const VIC_INTSEL: u32 = 3;
const VIC_INTENABLE: u32 = 4;
const VIC_INTCLEAR: u32 = 5;
const VIC_PROTECTION: u32 = 8;

const NUM_INTSRC: u32 = 32;

// ISR type alias: a function that takes (*mut trapframe, int) and returns ()
pub type ISR = fn(*mut arm_h::trapframe, u32);
...
fn default_isr(_tf: *mut arm_h::trapframe, n: u32) {
...
/// Panics on invalid `n`.
pub unsafe fn pic_enable(n: u32, isr: ISR) {
    if n < 0 || (n as u32) >= NUM_INTSRC {
...
pub unsafe fn pic_disable(n: u32) {
    if n < 0 || (n as u32) >= NUM_INTSRC {
